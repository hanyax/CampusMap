...
public class HW6TestDriver {
    ...
    private void addNode(String graphName, String nodeName) {
    	...
    	DirectedLabeledGraph<String, String> myGraph = graphs.get(graphName);
    ...
    private void listChildren(String graphName, String parentName) {
    	...
    	if (myGraph.getAllParents().contains(parentName)) {
	    	Map<String, HashSet<DirectedLabeledEdge<String, String>>> children = myGraph.getAllEdges(parentName);
	    	
	    	ArrayList<String> sortedChildren = new ArrayList<String>(children.keySet());
	    	Collections.sort(sortedChildren);
	    	for (String child : sortedChildren) {
	    		ArrayList<DirectedLabeledEdge<String, String>> sortedEdges= new ArrayList<DirectedLabeledEdge<String, String>>(myGraph.getAllEdges(parentName).get(child));
	    		Collections.sort(sortedEdges);
	    		for (DirectedLabeledEdge<String, String> edge : sortedEdges) {
	    			output.print(" " + edge.getChildrenNode() + "(" + edge.getLabel() + ")");
	    		}
	    	}
	    	output.println();
    	}
    ...
    private void findPath(String graphName, String startNode, String endNode) {
        ...
        // ___ = graphs.get(graphName);
        // output.println(...);
    	DirectedLabeledGraph<String, String> builtGraph = graphs.get(graphName);
    	Scanner input = new Scanner(System.in);
    	ArrayList<String> enteredHeros = new ArrayList<String>();
    	enteredHeros.add(startNode);
    	enteredHeros.add(endNode);
        List<DirectedLabeledEdge<String, String>> foundPath = MarvelPaths.findPath(builtGraph, enteredHeros);
        if (!builtGraph.getAllParents().contains(startNode) && !builtGraph.getAllParents().contains(endNode)) {
	        ...
	        if (foundPath != null) {  
		        for (DirectedLabeledEdge<String, String> edge : foundPath) {
		        	output.println(edge.getParentNode() + " to " + edge.getChildrenNode() + " via " + edge.getLabel());
		        }
	        } else {
